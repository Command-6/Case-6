...
async def get_elemets():
    ...
    for row in rows:
        item = {
            ...
            "author": row[2],
            "disc": row[3],
            "file_1": row[4].tobytes().decode('utf-8'),
            "file_2": row[5].tobytes().decode('utf-8'),
            "file_3": row[6].tobytes().decode('utf-8'),
            "date": row[7],
            "photo": row[8]
        }
        items.append(item)
...
async def search_element(search: str):
    ...
    for row in rows:
        item = {
            ...
            "author": row[2],
            "disc": row[3],
            "file_1": row[4].tobytes(),
            "file_2": row[5].tobytes(),
            "file_3": row[6].tobytes(),
            "date": row[7],
            "photo": row[8]
        }
